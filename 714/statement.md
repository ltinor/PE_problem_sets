...
- 字符 `PE`：输出原题（$K = 50\,000$）的官方答案，科学计数法保留 13 位有效数字；
- 或一个整数 $K$：输出 $D(K)$ 的**精确十进制值**。

## 输出格式

- 输入 `PE` 时输出 `2.452767775565e20`；
- 输入整数 $K$ 时输出一行一个整数，即 $D(K)$ 的精确值。
...
注：$K = 5 \times 10^4$ 即原题 PE 点（精确值 $245276777556527592946$，
13 位有效数字科学计数法即官方答案 $2.452767775565e20$）。
