...
int main() {
    ...
    // 2-3 小数据 (n < 10000)
    for (int i = 1; i < 3; i++) {
        test_cases[i] = {rand_ll(1, 10000), rand_ll(1, 100)};
    }

    // 4-6 大数据 (n 接近上界 1e6)
    for (int i = 3; i < 6; i++) {
        test_cases[i] = {rand_ll(1000000 - 100000, 1000000), rand_ll(1, 100)};
    }

    // 7-8 边界条件
    test_cases[6] = {1, 1};              // 最小值
    test_cases[7] = {1000000, 100};      // 最大值

    // 9-10 随机数
    for (int i = 8; i < 10; i++) {
        test_cases[i] = {rand_ll(1, 1000000), rand_ll(1, 100)};
    }
